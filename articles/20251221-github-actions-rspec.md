...
今回は Rails の CI 環境としては珍しく Oracle Database を使用していますが、これは本プロダクトの生い立ちによるものです。元々は高いパフォーマンスと堅牢性が求められる領域で、グループ会社によって開発されたシステムでした。事業統合により LITALICO が開発を引き継いだ現在も、当時の安定性重視の選定を尊重し、この構成で運用しています。
